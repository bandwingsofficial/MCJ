...
function ConfigValueBadge({ children }: { children: string }) {
  if (!children || children === "—") {
    return (
      <span className="text-sm font-semibold leading-snug text-[#0B1F3A]">—</span>
    );
  }

  return (
    <span className="inline-flex max-w-full items-center rounded-md border border-[#C7D9F5] bg-gradient-to-r from-[#EFF6FF] to-[#F0F5FF] px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-[#1E49A8]">
...
function ConfigRow({
  ...
  return (
    <div className="grid grid-cols-[6.75rem_minmax(0,1fr)] items-start gap-2 text-sm sm:grid-cols-[7.5rem_minmax(0,1fr)]">
      <span className="pt-1 text-xs font-medium text-slate-500 sm:text-sm">
        {label}
      </span>
      <div className="min-w-0 pt-0.5">
        <ConfigValueBadge>{value}</ConfigValueBadge>
      </div>
    </div>
  );
}
