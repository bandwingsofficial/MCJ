...
const FACULTY_MENU: MenuSection[] = [
  {
    section: "MAIN",
    items: [
      { name: "Dashboard", icon: LayoutDashboard, path: "/dashboard" },
...
const INTERVIEWER_MENU: MenuSection[] = [
  {
    section: "MAIN",
...
function menuForRole(role?: string): MenuSection[] {
  if (!role) return [];
...
export function BranchSidebar() {
  ...
  const portalLabel = formatRoleLabel(user?.role) || "Branch Portal";
  ...
  return (
    <>
      ...
      >
        ...
        <div
          className={cn("shrink-0 pb-6", collapsed ? "p-2" : "px-3 pt-3")}
        >
