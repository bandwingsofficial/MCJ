...
import type { JobApplicationBranchInterview } from "@/src/features/branch-ops/types";
import {
  formatAppliedDate,
...
} from "@/src/features/interviews/utils/job-application-interview-timeline.utils";
import {
...
type InterviewRow = JobApplicationBranchInterview;

function roundStepTitle(interview: InterviewRow): string {
  const order = interview.round?.sortOrder ?? interview.roundNumber ?? 0;
...
function InterviewRoundBody({
  ...
  const statusLabel = formatInterviewStatusLabel(interview.status);
  const clearedAt =
    interview.status === "COMPLETED" && interview.updatedAt
      ? formatInterviewDateTime(interview.updatedAt)
      : null;
  ...
  if (resultLabel && resultLabel !== "Pending") {
...
export type BranchApplicationTimelineSource = {
  status?: string;
  createdAt?: string;
...
interface Props {
  application: BranchApplicationTimelineSource;
}

export function BranchJobApplicationInterviewTimeline({ application }: Props) {
  const interviews = application.interviews ?? [];
  const roundEntries = buildInterviewTimelineByRound(interviews);
  const currentActiveId = pickWorkflowActiveInterviewId(interviews);
  ...
  steps.push({
    ...
    title: "Application",
    body: (
      <div className="mt-2 space-y-1.5 rounded-lg border border-slate-100 bg-slate-50/80 px-3 py-2.5">
        {application.createdAt ? (
          <DetailLine
            label="Applied"
            value={formatAppliedDate(application.createdAt)}
          />
        ) : null}
        {application.status ? (
          <DetailLine
  ...
  const assignmentInterview = [...interviews]
    .filter(
      (item) =>
        item.status === "ASSIGNED" ||
        (item.branch && item.interviewer?.id),
    )
    .sort(
      (a, b) =>
  ...
  if (application.branchAssignedAt || assignmentInterview) {
    const interviewerName = assignmentInterview
      ? getInterviewerName(
          assignmentInterview as NonNullable<
            import("@/src/features/branch-ops/types").JobApplicationItem["latestInterview"]
          >,
        )
      : null;
    steps.push({
      key: "assignment",
      state: currentActiveId ? "done" : "pending",
      title: "Assignment",
      body: (
        <div className="mt-2 space-y-1.5 rounded-lg border border-slate-100 bg-slate-50/80 px-3 py-2.5">
          {application.branchAssignedAt ? (
            <DetailLine
              label="Assigned At"
              value={formatInterviewDateTime(application.branchAssignedAt)}
            />
          ) : null}
          {assignmentInterview?.branch ? (
            <DetailLine
              label="Branch"
              value={`${assignmentInterview.branch.branchName} (${assignmentInterview.branch.branchCode})`}
            />
          ) : null}
          {interviewerName ? (
            <DetailLine label="Interviewer" value={interviewerName} />
          ) : null}
          <DetailLine label="Assignment Status" value="Assigned" />
        </div>
      ),
    });
  }
  ...
  roundEntries.forEach((entry) => {
    ...
    steps.push({
      key: entry.roundKey,
      state,
      title: roundStepTitle(interview),
      body: (
  ...
  if (steps.length <= 1 && roundEntries.length === 0) {
    return (
