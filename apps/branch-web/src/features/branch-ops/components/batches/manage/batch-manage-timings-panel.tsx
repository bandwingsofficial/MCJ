...
import {
  getBatchModeLabel,
  getConfiguredBatchModeSummaries,
  getTimingsForMode,
...
export function BatchManageTimingsPanel({ batch }: Props) {
  ...
  const modeSummaries = getConfiguredBatchModeSummaries(batch);
        ...
        modeSummaries.map((modeRow) => {
          ...
          return (
            ...
            >
              {timings.length === 0 ? (
                <BatchManageEmptyMessage
                  message={`No ${getBatchModeLabel(modeRow.mode)} timings are linked to this batch.`}
                />
              ) : (
